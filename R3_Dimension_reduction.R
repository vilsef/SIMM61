...
# Lot of items are noted to have a moderate to high skewness. This could 
# be a problem for our analysis, but for the sake of our time frame, we 
# choose to leave this as it is. 
...
#p-value of these tests is lower than 0.05, this indicates the violation of the 
...
# Iteration 2: The communality table shows that ar6: "ar 6.    Most medical research done on animals is unnecessary and invalid. " 
# is the best represented item in the 2-factor structure,
# with 67% of its total variance explained by the new factors. 
# Ar 8 on the other hand: "Pet owners are responsible for preventing their pets from killing other animals, 
# such as cats killing mice or snakes eating live mice",
# is the least represented item, with only 14% of its variance explained by the current factor structure.
#################
...
# We here choose the Oblique promax rotation method as it assumes that the factors 
...
# are correlated.
EFA_mod2 <- fa(df_Cor, nfactors = 2, fm = "pa", rotate = "promax")
...
vss(df_Cor2)
EFA_scree2 <- PCA(df_items_only2) # Model for Scree test
...
EFA_mod3 <- fa(df_Cor2, nfactors = 2, fm = "pa", rotate = "promax")
...
# will be adressed in our final discussion. # We now choose to name our two factors:
# Factor 1: "Animal Exploitation", number 2: "Animal research". 
